...
const ROLE_ENUM = ['worker','group-leader','project-manager','manager','admin','superadmin'];
...
function syncRoleFields(doc) {
  ...
  const primary = normalizeRole(
    doc.role ||
    (Array.isArray(doc.roles) && doc.roles.length > 0 ? doc.roles[0] : 'worker')
  );
...
const BiometricSummarySubSchema = new Schema(
  {
    status: {
      type: String,
      enum: ['not-enrolled','pending','enrolled','rejected','revoked','expired'],
...
UserSchema.index(
  ...
  { unique: true, sparse: true, partialFilterExpression: { staffNumber: { $type: 'string' } } }
);
...
UserSchema.pre('findOneAndUpdate', async function (next) {
  try {
    ...
    if (Array.isArray(setObj.roles)) setObj.roles = setObj.roles.map(normalizeRole);

    // Keep fields in sync
    if (setObj.role != null && (!setObj.roles || setObj.roles.length === 0)) {
      setObj.roles = [setObj.role];
    } else if ((setObj.role == null || setObj.role === '') && Array.isArray(setObj.roles) && setObj.roles.length > 0) {
      setObj.role = setObj.roles[0];
    } else if (setObj.role != null && Array.isArray(setObj.roles) && setObj.roles.length > 0) {
      setObj.role = setObj.roles[0];
    }
