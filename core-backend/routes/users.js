...
function orgFilterFromReq(model, req) {
  ...
  if (p.instance === "ObjectId") {
    if (explicit && explicit instanceof mongoose.Types.ObjectId)
      return { orgId: explicit };
...
function mintPhotoObjectId(userId, mimeType) {
  const ext =
    mimeType && mimeType.includes("jpeg")
      ? "jpg"
...
function forbidSuperadminUnlessGlobal(req, incomingRole) {
  ...
  if (role === "superadmin" && req.user?.isGlobalSuperadmin !== true) {
    const err = new Error(
      'Only a platform global superadmin can assign the role "superadmin"',
    );
    err.status = 403;
    throw err;
  }
...
router.get("/", async (req, res) => {
  try {
    ...
    const missingPhoto =
      String(req.query.missingPhoto || "").toLowerCase() === "true";
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const normUsername =
        username != null ? String(username).trim() : undefined;
      const normStaff =
        staffNumber != null ? String(staffNumber).trim() : undefined;
      ...
      const doc = new User({
        ...
        biometric: { status: "not-enrolled", lastUpdatedAt: new Date() },
        isDeleted: false,
      });
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const normStaff =
        staffNumber !== undefined ? String(staffNumber).trim() : undefined;
  ...
  async (req, res) => {
    try {
      ...
      if (!password || String(password).length < 6) {
        return res
          .status(400)
          .json({ error: "password required (min 6 chars)" });
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const objectId = mintPhotoObjectId(
        user._id,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!objectId)
        return res.status(400).json({ error: "objectId required" });
      if (!mongoose.Types.ObjectId.isValid(id))
        return res.status(400).json({ error: "invalid user id" });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const token = Buffer.from(`${user._id}:${Date.now()}`).toString(
        "base64url",
      );
      const policy = {
...
router.post("/:id/biometric/submit", async (req, res) => {
  try {
    ...
    if (!templateVersion)
      return res.status(400).json({ error: "templateVersion required" });
    if (!embedding)
      return res.status(400).json({ error: "embedding (base64) required" });
    ...
    user.biometric = {
      ...
      lastLivenessScore:
        typeof livenessScore === "number"
          ? livenessScore
          : user.biometric?.lastLivenessScore,
      lastUpdatedAt: new Date(),
    };
    await user.save();
    ...
    try {
      ...
      await BiometricEnrollment.updateMany(
        ...
        },
        {
          $set: {
            status: "revoked",
            revokedBy: asOid(req.user?._id),
            revokedAt: new Date(),
            revokeReason: reason ? String(reason) : undefined,
          },
        },
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        ok: true,
        user: stripSecrets(u.toObject({ versionKey: false })),
      });
    } catch (e) {
  ...
  async (req, res) => {
    try {
      ...
      if (!orgScope.orgId) {
        return res
          .status(400)
          .json({ error: 'orgId missing/invalid; include header "x-org-id"' });
      }
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const groupName = String(
          mapField(r, "groupname") || mapField(r, "group") || "",
        ).trim();
        ...
        if (!name || (!email && !username && !staffNumber)) {
          errors.push({
            row: i + 1,
            error: "Name and one of (email|username|staffNumber) required",
          });
          continue;
        }
        ...
        if (role === "superadmin" && req.user?.isGlobalSuperadmin !== true) {
          errors.push({
            row: i + 1,
            error: 'Role "superadmin" requires platform global superadmin',
          });
          continue;
        }
