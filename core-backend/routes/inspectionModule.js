...
function userRoles(reqUser) {
  ...
  const extras = Array.isArray(reqUser?.roles)
    ? reqUser.roles.map(normalizeRole)
    : [];
...
function canRunForm(reqUser, form) {
  const allowed = Array.isArray(form?.rolesAllowed)
    ? form.rolesAllowed.filter(Boolean)
    : [];
...
function scrubFormPayload(body = {}) {
  ...
  const rolesAllowed = Array.isArray(body.rolesAllowed)
    ? body.rolesAllowed.map(normalizeRole).filter(Boolean)
    : [];
  const audienceIn = body.audience || {};
  const audience = {
    userIds: Array.isArray(audienceIn.userIds)
      ? audienceIn.userIds.filter(Boolean)
      : [],
    groupIds: Array.isArray(audienceIn.groupIds)
      ? audienceIn.groupIds.filter(Boolean)
      : [],
  };
  ...
  const scoring =
    body.scoring && typeof body.scoring === "object"
      ? {
          mode: ["any-fail", "tolerance", "percent"].includes(
            String(body.scoring.mode || "").toLowerCase(),
          )
            ? String(body.scoring.mode).toLowerCase()
            : "any-fail",
          maxNonCriticalFails: Number.isFinite(
            +body.scoring.maxNonCriticalFails,
          )
            ? +body.scoring.maxNonCriticalFails
            : 0,
          minPassPercent: Number.isFinite(+body.scoring.minPassPercent)
            ? +body.scoring.minPassPercent
            : 100,
        }
  ...
  const subjType = allowedSubjects.includes(
...
function computeOverallWithScoring(form, items) {
  ...
  const nonCriticalFailCount = applicable.filter(
    (it) =>
      String(it.result || "").toLowerCase() === "fail" && !it.criticalOnFail,
  ).length;

  if (mode === "tolerance") {
    const maxFails = Number.isFinite(+scoring.maxNonCriticalFails)
      ? +scoring.maxNonCriticalFails
      : 0;
    return nonCriticalFailCount > maxFails ? "fail" : "pass";
  }

  if (mode === "percent") {
    const minPct = Number.isFinite(+scoring.minPassPercent)
      ? +scoring.minPassPercent
      : 100;
...
function roleRank(r) {
  return ROLE_ORDER[normalizeRole(r)] ?? 0;
}
...
router.get("/forms", async (req, res, next) => {
  try {
    ...
    if (projectId && String(projectId).trim()) {
      const projVal =
        wantsObjectId(InspectionForm, "scope.projectId") &&
    ...
    const rows = await InspectionForm.find(where)
      .select(
        "_id title description formType scope subject isDeleted scoring createdAt updatedAt rolesAllowed",
      )
      .sort({ updatedAt: -1 })
      .lean();
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!ensureOrgOnDoc(InspectionForm, doc, req))
        return res
          .status(400)
          .json({
            error:
              "orgId is required on InspectionForm; missing/invalid in token",
          });
      await doc.save();
...
router.get("/forms/:id", async (req, res, next) => {
  try {
    ...
    const where = {
      ...
      ...orgFilterFromReq(InspectionForm, req),
    };
...
async function validateAssessedUserForPerformance(req, subjectAtRun) {
  ...
  if ((subjectAtRun?.type || "none") !== "performance")
    return { ok: true, assessedUserId: undefined, label: "" };
  ...
  if (!id)
    return {
      ok: false,
      error: "assessed user id is required for performance inspections",
    };
  ...
  if (!u)
    return { ok: false, error: "assessed user not found in your organization" };
...
router.post("/forms/:id/run", async (req, res, next) => {
  try {
    ...
    if (!canRunForm(req.user, form))
      return res.status(403).json({ error: "Not allowed to run this form" });
    ...
    const tplById = {};
    (form.items || []).forEach((t) => {
      if (t && t._id) tplById[String(t._id)] = t;
    });

    const normalizedItems = itemsIn.map((inIt, idx) => {
      const tpl =
        (inIt?.itemId && tplById[String(inIt.itemId)]) || form.items[idx] || {};
      ...
      const correctiveAction =
        result === "fail" ? String(inIt?.correctiveAction || "") : "";
      ...
      if (result === "fail" && tpl.requireEvidenceOnFail === true) {
        ...
        if (!hasAny) {
          const e = new Error(`Item ${idx + 1} requires evidence on fail`);
          e.status = 400;
          throw e;
        }
      }
      if (result === "fail" && tpl.requireCorrectiveOnFail !== false) {
        if (!correctiveAction.trim()) {
          const e = new Error(
            `Item ${idx + 1} requires corrective action on fail`,
          );
          e.status = 400;
          throw e;
        }
    ...
    const subjectAtRun = (() => {
      ...
      if (
        ["vehicle", "asset", "performance"].includes(String(locked.type)) &&
        locked.lockToId
      ) {
        return {
          type: locked.type,
          id: locked.lockToId,
          label: String(locked.lockLabel || ""),
        };
      }
    ...
    const submission = new InspectionSubmission({
      ...
      subjectAtRun:
        subjectAtRun.type === "performance" && perf.label
          ? { ...subjectAtRun, label: perf.label }
          : subjectAtRun,
      assessedUserId:
        subjectAtRun.type === "performance" ? perf.assessedUserId : undefined,
      ...
      signoff: {
        ...
        name: String(
          body?.signoff?.name || req.user?.name || req.user?.email || "",
        ),
    ...
    if (
      submission.location &&
      submission.location.type === "Point" &&
      !Array.isArray(submission.location.coordinates)
    ) {
    ...
    if (!ensureOrgOnDoc(InspectionSubmission, submission, req))
      return res
        ...
        .json({
          error:
            "orgId is required on InspectionSubmission; missing/invalid in token",
        });
...
router.get("/submissions", async (req, res, next) => {
  try {
    ...
    if (subjectType)
      where["subjectAtRun.type"] = String(subjectType).toLowerCase(); // 'vehicle'|'asset'|'performance'|'none'
    if (subjectId) where["subjectAtRun.id"] = subjectId;

    if (
      assessedUserId &&
      mongoose.Types.ObjectId.isValid(String(assessedUserId))
    ) {
      where.assessedUserId = new mongoose.Types.ObjectId(
        String(assessedUserId),
      );
    }
    ...
    const rows = await InspectionSubmission.find(where)
      .select(
        "_id formId formTitle formType scopeAtRun overallResult createdAt runBy links subjectAtRun assessedUserId signoff.confirmed isDeleted scoringSummary location locationMeta",
      )
      .sort({ createdAt: -1 })
      .limit(lim)
      .lean();
...
router.get("/submissions/:id", async (req, res, next) => {
  try {
    ...
    const where = {
      _id: new mongoose.Types.ObjectId(id),
      ...orgFilterFromReq(InspectionSubmission, req),
    };
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const sub = await InspectionSubmission.findOneAndUpdate(
        where,
        { isDeleted: false },
        { new: true },
      );
  ...
  try {
    ...
    for (const u of rows) {
      ...
      if (anyRole.some((r) => isRoleAtLeast(r, minRole))) {
        const txt =
          `${u.name || ""} ${u.email || ""} ${u.username || ""}`.toLowerCase();
        if (!q || txt.includes(q)) {
          filtered.push({
            _id: u._id,
            name: u.name,
