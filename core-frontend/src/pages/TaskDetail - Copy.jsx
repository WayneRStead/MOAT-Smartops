...
export default function TaskDetail() {
  ...
  async function computeEffectiveFences(taskId, pid) {
    if (taskFenceApi !== "absent") {
      try {
        const t = await api.get(`/tasks/${taskId}/geofences`, { headers: { "cache-control": "no-cache" }});
    ...
    if (pid) {
      try {
        const p = await api.get(`/projects/${pid}/geofences`, { headers: { "cache-control": "no-cache" }});
  ...
  async function load() {
    ...
    try {
      ...
      setNewAction("start");
      setNewAt(toLocalDateInputValue(new Date()));
      setNewNote("");
      setEditId(null);
      setFile(null);
      setFileNote("");
  ...
  // ---- Pin/enforcement save (unchanged behavior) ----
  ...
  // ---- Actions / Logs / Attachments (unchanged from before) ----
    ...
    <div className="p-4 space-y-4">
                ...
                onChange={(e) => {
                  ...
                  if (String(v || "") !== String(lastGoodTaskRef.current?.projectId || "")) {
                    optimisticSave(
                      ...
                      // refresh effective fences (project might change)
      ...
      <div className="border rounded p-3 space-y-3">
        ...
        <SafeGeoFencePreview
          ...
          taskId={taskFenceApi === "present" ? id : undefined /* avoid GET 404s */}
          showTaskCoverage={true}
          height={320}
          className="rounded"
